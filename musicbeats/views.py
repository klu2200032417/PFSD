...
from musicbeats.models import search, Song, Watchlater, History, Channel
...
def search(request):
    ...
    return render(request, 'musicbeats/search.html', {"songs": songs})
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return redirect('index')  # Assuming 'index' is the name of the URL pattern for the default page
...
def delete_song(request):
    if request.method == 'POST':
        ...
        try:
            song_to_delete = Song.objects.get(name=music_name)
            song_to_delete.delete()
            # Optionally, you can add a success message here
            return redirect('songs')  # Redirect to the songs page after deletion
        except Song.DoesNotExist:
            # Handle the case where the song doesn't exist
            pass
    # If the request method is not POST or the song doesn't exist,
    # render the delete.html template
    return render(request, 'musicbeats/delete.html')
